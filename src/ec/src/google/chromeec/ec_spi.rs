...
use drivers::{context::Context, spi::spi_generic::{SPICtrlrBuses, SPISlave}};
...
pub fn crosec_spi_io(req_size: usize, resp_size: usize, ctx: &mut dyn Context) -> Result<(), Error> {
    ...
    if let Some(slave) = ctx.as_any_mut().downcast_mut::<SPISlave>() {
        ...
        if slave.xfer(&(*REQ_BUF.read()).data[..req_size], &mut [0u8; 0]).is_err() {
            println!("{}: Failed to send request.", FUNC_NAME);
            return out(&slave);
        }
        ...
        loop {
            ...
            if slave.xfer(&[0u8; 0], &mut byte[..]).is_err() {
                println!("{}: Failed to receive byte", FUNC_NAME);
                return out(&slave);
            }
            ...
            if sw.expired() {
                println!("{}: Timeout waiting for framing byte.", FUNC_NAME);
                return out(&slave);
            }
        }

        if slave.xfer(&[0u8; 0], &mut (*RESP_BUF.write()).data[..resp_size]).is_err() {
            println!("{}: Failed to receive a response.", FUNC_NAME);
        }
...
pub fn google_chromeec_command(cec_command: &mut ChromeECCommand, spi_map: &[SPICtrlrBuses]) -> Result<(), Error> {
    static DONE: RwLock<bool> = RwLock::new(false);
    static SLAVE: RwLock<SPISlave> = RwLock::new(SPISlave::new());

    if !*DONE.read() {
        (*SLAVE.write()).setup(SPI_BUS, SPI_CHIP, spi_map).map_err(|e| Error::ECSPIError(e))?;
...
pub fn google_chromeec_get_event() -> HostEventCode {
    println!("{}: Not supported.", "google_chromeec_get_event");
    HostEventCode::None
}
