...
impl AcpiGen {
    ...
    pub fn write_len_f(&mut self) -> Result<(), Error> {
        ...
        if self.len_stack.len() <= self.ltop {
            self.len_stack.push(self.gencurrent.clone());
        } else {
    ...
    pub fn get_stack_current(&self) -> &str {
        if self.len_stack.len() > self.ltop {
            &self.len_stack[self.ltop]
        } else {
            &self.gencurrent
        }
    }
    ...
    pub fn emit_byte(&mut self, c: u8) -> Result<(), Error> {
        self.gencurrent.push(c as char);
        Ok(())
    }
